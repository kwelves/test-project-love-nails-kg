...
export function TeamDrawer({ member, isOpen, side = "right", onClose }: TeamDrawerProps) {
  ...
  const moveWork = (direction: 1 | -1) => {
    setActiveWork(
      (current) =>
        (current + direction + member.workImages.length) %
        member.workImages.length
    );
  };
  ...
  const whatsappUrl = `https://wa.me/${salon.contact.whatsappPhone}?text=${encodeURIComponent(
    whatsappMessage
  )}`;
  ...
  const panelVariants = {
    hidden: isMobile ? { y: "100%" } : { x: side === "right" ? "100%" : "-100%" },
    visible: isMobile ? { y: 0 } : { x: 0 },
    exit: isMobile ? { y: "100%" } : { x: side === "right" ? "100%" : "-100%" },
  };
        ...
        <>
          ...
          <motion.div
            ...
            className={cn(
              ...
              isMobile
                ? "bottom-0 left-0 right-0 max-h-[85vh] rounded-t-[1.5rem]"
                : cn(
                    "inset-y-0 w-[650px]",
